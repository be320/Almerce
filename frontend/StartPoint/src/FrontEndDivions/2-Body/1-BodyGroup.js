  ...
  const handleChoiceClick = (event, choiceType) => {
    ...
    if (event.target.innerText === "عودة") {
      ...
      props.actions.clientSideActions.sendNlpTemplate({
        ...
        choiceType: choiceType,
        queries:
          "انا كنت بدور علي لعبة بازل لبنتي لصور حيوانات تساعدها علي التدقيق و التركيز و تشغيل العقل و تكون مناسبة لطفل عندة 4 سنين",
      });
      localStorage.setItem("index", JSON.stringify(temp + 1));
    } else {
      props.actions.clientSideActions.sendOneWayTemplate({
    ...
    <>
      <div
        ...
        style={{
          height: `calc(100vh - ${
            props.footerSize.height +
            (props.selectedImages.length === 0 ? 0 : 150)
          }px)`,
        }}
      >
        {props.bodyContainer.map((item) => {
          if (item.elementType === "MessageTemplate") {
            ...
            return <VoiceNoteTemplate Audio={item.audio} />;
          } else if (item.elementType === "ImageTemplate") {
            return item.imageList.map((subitem) => (
              <ImageTemplate image={subitem.imageURL} />
            ));
