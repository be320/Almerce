...
import React, { useState ,useRef, useEffect } from "react"
 ...
 const  SimpleRating = (props) => {
  ...
  return<>
    ...
    {(value===0) && (<Rating name="simple-controlled" precision={0.5} value={value} onChange={(event,newValue) => {props.changeRating(event,newValue)
    setValue(newValue)}}/>)}
    {(value!==0) && ( <Rating name="read-only"  precision={0.5} value={value} readOnly />)}
    </div>
    </div>
  </div>
</>
